...
export const Sidebar: FC<IProps> = ({ sidebarLinks, pathname, isVerticalIcon, isMobileSidebar }) => {
        ...
        return (
            <>
                <aside
                    ...
                    style={{ backgroundImage: `linear-gradient(90deg, #5429E8 0.12%, #70369A 99.97%)` }}
                >
                        ...
                        <ul className='flex justify-between items-center px-16'>
                            {
                                sidebarLinks.map((link: ISidebar, index: number) => {
                                    return (
                                        <SidebarMobileItem pathname={pathname} key={`link-${index}-${link.path}`} sidebarLinks={link} />
                                    )
                                })
                            }
                        </ul>
