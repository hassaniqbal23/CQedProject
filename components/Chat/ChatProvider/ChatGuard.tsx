...
import { EVENT_BUS_ADD_NEW_INCOMING_MESSAGE_TO_INBOX_RESPONSE, JOIN_TO_CHAT_ROOM } from '../EventBus/constants';
...
export const ChatGuardProvider = ({ children }: any) => {
    ...
    useEffect(() => {
        if (socket) {
            ...
            //     if (!realtimeConnectedUsersIds.includes(userId)) {
            //         setRealtimeConnectedUsersIds([...realtimeConnectedUsersIds, userId]);
            //     }
            // });
            // user_disconnected
    ...
    const sendMessage = (message: { ['key']: string | number | unknown }) => {
        if (socket) {
            console.log(message)
