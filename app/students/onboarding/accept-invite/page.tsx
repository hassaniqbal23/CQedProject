...
import {
    AcceptStudentsInvitation,
    IAcceptStudentsInvitation,
} from '@/app/api/students';
...
export default function TeacherAcceptInvite({
    ...
    const { mutate: acceptTeacherInvitation, isLoading: isAcceptingEmail } =
        useMutation(
            (userData: IAcceptStudentsInvitation) => AcceptStudentsInvitation(userData),
            {
                onSuccess: (res) => {
                    toast.success(res.data.message);
                    const response = res;
                    setIsLoading(false);
                    router.push('/students/onboarding/create-profile');
